  ...
  <Modal :close-on-click-outside="true" @close="emit('close')">
    <div class="flex flex-col gap-6 w-full text-left">
      ...
      <div class="flex flex-col gap-4">
        ...
        <div class="flex items-center gap-2" v-if="tags.length < 3">
          ...
          <Button @click="emit('addTag', newTag)" :disabled="!newTag.trim()">
            Añadir
          </Button>
        </div>

        <div
          v-if="allTags.filter((t) => !tags.includes(t.name.toLowerCase())).length > 0"
